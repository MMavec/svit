<script lang="ts">
	...
	// Move the overlay to <body> so its `position: fixed` anchors to the viewport.
	// Panels use `backdrop-filter` (frosted glass), which establishes a containing
	// block for fixed descendants — without this, the drawer anchors to the panel
	// box and can land off-screen above the top edge.
